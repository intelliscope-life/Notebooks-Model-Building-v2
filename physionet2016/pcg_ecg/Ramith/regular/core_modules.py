...
def is_blank(x):
    img = cv.imread(pcg_dataset_path + x, 0)
...
def set_dataset_path(path1,path2,readtype,ignore_folder=None):
    global Read_Type
    Read_Type = readtype
    
    global pcg_dataset_path
    pcg_dataset_path = path1

    global ecg_dataset_path
    ecg_dataset_path = path2
    
    global images
    form = '.tiff'
    images = sorted(os.listdir(pcg_dataset_path))
...
def show_i(x):
    img = cv.imread(pcg_dataset_path + images[x], 0)
...
def load_hybrid_data(leave_test = False, test_set=None):
    ...
    global ecg_dataset_path
    ecg_files = os.listdir(ecg_dataset_path)
    
    test_records =get_csv_names('test_records.csv', ecg_dataset_path, pcg_dataset_path) # 'other_records.csv/ test_records.csv'
    ...
    for i in range(0,n):
        ...
        im1 = cv.imread(pcg_dataset_path + images[i], Read_Type)
        im2 = cv.imread(ecg_dataset_path + images[i], Read_Type)
...
def load_data(train_percentage):
    ...
    if(len(pcg_dataset_path)==0):
        print("Dataset path not set")
        return 
    
    for i in range(0,n):
        img = cv.imread(pcg_dataset_path + images[i], Read_Type)
...
def load_balanced_hybrid_data(leave_test = False, test_set=None):
    ...
    ecg_dataset_path  = pcg_dataset_path[:-4] + 'ecg/'
    ecg_files = os.listdir(ecg_dataset_path)
    ...
    for i in range(0,n):
        im1 = cv.imread(pcg_dataset_path + s_images[i], Read_Type)
        im2 = cv.imread(ecg_dataset_path     + s_images[i], Read_Type)
