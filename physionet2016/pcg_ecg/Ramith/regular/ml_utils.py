...
def log_epoch_data(logs, npt):
    npt.log_metric('epoch/sparse_categorical_accuracy', logs['sparse_categorical_accuracy'])
    npt.log_metric('epoch/loss', logs['loss'])
    npt.log_metric('epoch/val_sparse_categorical_accuracy', logs['val_sparse_categorical_accuracy'])
    npt.log_metric('epoch/val_loss', logs['val_loss'])
...
def log_model_weights(model, epoch, logs, npt):
    with tempfile.TemporaryDirectory(dir='.') as d:
        ...
        model_dir = 'epoch-{}--acc-{:0.3f}'.format(epoch, logs['sparse_categorical_accuracy'])
...
def lr_scheduler(epoch, lr, npt):
    if epoch < 20:
        ...
        new_lr = lr * np.exp(0.005 * (20 - epoch))
